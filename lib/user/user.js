...
module.exports = class User {
    ...
    async updateUserDB() {
        try {
            ...
            console.log(`Cannot update userdb for ${this.#uuid}, error: ${e}`);
        }
        return false;
    }
    ...
    async #getLoginSession(key, keyType) {
        try {
            ...
            if (((keyType === 'session') && (key === this.#sessionId)) || (keyType === 'password') && (User.verifyPassword(key, this.#password))) {
     ...
     * @returns {Promise<boolean>}
     */
    async isValidSession(sid) {
        return this.#sessionId === sid && !this.isInactive();
    }

    /**
     * Update users last activity timestamp.
     */
    setActivity() {
     ...
     */
    isInactive() {
        return ((Date.now() - this.#lastActivity) / (60 * 1000)) >= 15;
    }

    /**
     * Destroy users sessionId.
     */
    destroySession() {
        this.#sessionId = '';
        this.updateUserDB();
